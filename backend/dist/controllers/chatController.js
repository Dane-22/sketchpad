...
exports.chatController = {
    ...
    getProjectChannels: async (req, res) => {
        try {
            ...
            let channels = await db_1.prisma.chatChannel.findMany({
                where: { projectId },
                include: {
                    members: {
                        include: {
            ...
            if (!channels || channels.length === 0) {
                const generalChannel = await db_1.prisma.chatChannel.create({
                    data: {
                        projectId,
                        name: 'general',
                ...
                const aiChannel = await db_1.prisma.chatChannel.create({
                    data: {
                        projectId,
                        name: 'engi-ai',
                ...
                channels = await db_1.prisma.chatChannel.findMany({
                    where: { projectId },
                    include: {
                        members: {
                            include: {
                        ...
                        create: [
                            ...
                            ...(Array.isArray(memberIds)
                                ? memberIds
                                    ...
                                    .map((uid) => ({ userId: uid, role: 'MEMBER' }))
                                : [])
                        ]
                    }
                },
    ...
    sendMessage: async (req, res) => {
        try {
            ...
            // Save user message
            const savedUserMessage = await db_1.prisma.chatMessage.create({
                data: {
                    channelId,
                    userId: user.id,
                    isAi: false,
            ...
            if (isAiTriggered) {
                ...
                savedAiMessage = await db_1.prisma.chatMessage.create({
                    data: {
                        channelId,
                        isAi: true,
    ...
    addChannelMembers: async (req, res) => {
        try {
            const { channelId } = req.params;
            ...
            for (const uid of userIds) {
                await db_1.prisma.groupMember.upsert({
                    where: {
                        channelId_userId: {
                            channelId,
                            userId: uid
            ...
            const updatedMembers = await db_1.prisma.groupMember.findMany({
                where: { channelId },
                include: {
                    user: {
                        select: { id: true, fullName: true, role: true, email: true }
    ...
    removeChannelMember: async (req, res) => {
        try {
            ...
            await db_1.prisma.groupMember.deleteMany({
                where: {
                    channelId,
                    userId
                }
...
Format responses cleanly using Markdown (bolding, bullet points, tables where suitable). Keep answers concise, technical, and actionable.`;
    ...
    if (context) {
        ...
        if (context.elementCount !== undefined)
            drawingContextText += `Total Canvas Elements: ${context.elementCount}\n`;
        if (context.elementTypes)
            drawingContextText += `Entity Types: ${JSON.stringify(context.elementTypes)}\n`;
    ...
    if (lower.includes('summar') || lower.includes('comment') || lower.includes('discuss')) {
        return `### 📋 Discussion & Drawing Summary\n\n- **Active Pins**: ${context?.comments?.length || 0} pinned topic(s)\n- **Entities**: ${context?.elementCount || 0} drawing elements\n\n**Key Discussion Points:**\n${context?.comments && context.comments.length > 0
            ? context.comments.map((c, i) => `- **Point ${i + 1} (${c.isResolved ? '✅ Resolved' : '⏳ Open'})**: ${c.user}: _"${c.content}"_`).join('\n')
